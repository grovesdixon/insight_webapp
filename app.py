...
df = pd.DataFrame({'x':patient, 'percentage':pcts, 'prediction':names})
fig = px.bar(df, x="percentage", y="x", color='prediction', orientation='h',
             height=300,
             hover_data=["prediction", "percentage"],
             color_discrete_map={'readmission':'firebrick',
                                  'safe':'darkgrey'})
fig.update_layout(
    title='Probability readmission within 30 days of discharge',
    yaxis_title="",
